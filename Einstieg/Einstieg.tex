...
In der \LaTeX-Datei zu diesem Dokument wird mit dem Befehl \texttt{\textbackslash usepackage\{babel\}} ein Packet geladen, welches es ermöglicht, die verwendete Sprache im Dokument zu ändern. 
Dies hat Auswirkungen auf die Bezeichnungen von automatisch erstellten Bezeichnungen und Überschriften wie z.B. Inhaltsverzeichnes, Tabellverzeichnis usw. sowie die Silbentrennung. 
Mit der Option \texttt{ngerman} wird deutsch als Sprache ausgewählt. 
\\ 
Um Umlaute und ß verwenden zu können, und damit diese im Editor als ein Zeichen dargestellt werden, müssen die Pakete \texttt{inputenc} und \texttt{fontnc} geladen werden.
Durch das einbinden von \texttt{\textbackslash usepackage{lmodern}} wird die Schriftfamilie latin-modern ausgewählt. Schließlich wird mittels \texttt{\textbackslash usepackage\{geometry\}} das Geometry-Packet geladen, welches Funktionen zur Einstellung der Seiten-Geometrie bereitstellt. 
 %
\subsection{Dokumentkörper}
%
Nachdem alle die Dokumentklasse festgelegt und alle benötigten Pakete geladen worden, kann endlich Inhalt produziert werden.
